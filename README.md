# 🏥 健康计算器 - 免费在线健康工具

[![Website](https://img.shields.io/badge/Website-Online-brightgreen)](https://cuibingyan-sys.github.io/1)
[![License](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Deploy](https://img.shields.io/badge/Deploy-Vercel-black)](https://vercel.com)

> 一站式免费在线健康计算工具平台，提供 BMI、卡路里、体脂率、基础代谢率等专业健康指标计算，助您科学管理身体数据。

## 🌐 在线访问

**[https://cuibingyan-sys.github.io/1](https://cuibingyan-sys.github.io/1)**

## ✨ 核心功能

| 工具 | 说明 | 适用场景 |
|------|------|---------|
| **BMI 计算器** | 身体质量指数计算，快速判断体重状况 | 日常健康监测 |
| **每日卡路里计算器** | 根据年龄/性别/活动水平计算每日热量需求 | 减肥/增肌 |
| **基础代谢率 (BMR)** | 计算静息状态下消耗的热量 | 科学饮食规划 |
| **体脂率估算器** | 基于身体测量数据估算体脂率 | 精准身体评估 |
| **理想体重计算器** | 综合多种医学公式计算健康体重范围 | 目标设定 |

## 📚 健康知识库

- 🔥 [2026夏季减肥全攻略](https://cuibingyan-sys.github.io/1/summer-weight-loss) - 科学减脂不反弹的终极指南
- 💪 [2026科学减肥完整指南](https://cuibingyan-sys.github.io/1/weight-loss-guide) - 一个月瘦10斤不反弹
- 🥛 [蛋白粉选购指南](https://cuibingyan-sys.github.io/1/protein-powder-guide) - 健身增肌减脂必备
- 📊 [BMI完全指南](https://cuibingyan-sys.github.io/1/bmi-guide) - 了解你的身体质量指数
- 🍽️ [卡路里完全指南](https://cuibingyan-sys.github.io/1/calorie-guide) - 科学饮食从这里开始

## 🛒 好物推荐

- ⚖️ [智能体脂秤对比评测](https://cuibingyan-sys.github.io/1/smart-scale-compare) - 华为/小米/Keep/香山全方位对比
- 🛍️ [推荐好物商城](https://cuibingyan-sys.github.io/1/shop) - 京东联盟精选健康好物

## 🛠 技术栈

- **前端**: HTML5 + CSS3 + Vanilla JavaScript
- **部署**: Vercel (自动部署)
- **分析**: Google Analytics 4 + 百度统计
- **广告**: Google AdSense + 京东联盟
- **SEO**: Google Search Console + 百度站长平台 + Sitemap
- **通知**: 飞书机器人 (DeepSeek V4 驱动)
- **自动化**: GitHub Actions (每日数据报告)

## 📊 数据面板

访问 [Dashboard](https://cuibingyan-sys.github.io/1/dashboard) 查看实时流量和收益数据。

## 🚀 快速开始

```bash
# 克隆仓库
git clone https://github.com/Cuibingyan-sys/1.git

# 进入目录
cd 1

# 使用任意静态服务器运行
# 例如使用 Python:
python3 -m http.server 8080

# 或使用 npx serve:
npx serve .
```

## 📈 变现方式

- **Google AdSense** - 自动广告展示
- **京东联盟** - 健康好物推荐佣金
- **支付宝/微信打赏** - 用户自愿支持

## 📝 更新日志

### v4.0 (2026-07-17)
- 新增好物推荐页 + 夏季减肥攻略
- 网站全面修复优化
- 飞书机器人升级至 v4.0 (DeepSeek V4 驱动)

### v3.0 (2026-07-17)
- 百度统计 + 二维码修复
- UI/UX 全面升级
- 京东联盟配置 + 监控完善

## 📄 许可证

MIT License

## 👤 作者

**Cuibingyan-sys** - [GitHub](https://github.com/Cuibingyan-sys)

---

⭐ 如果这个项目对您有帮助，请给个 Star 支持一下！